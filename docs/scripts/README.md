...
**`-WhatIf` on anything that writes.** `Import-GuestPermissions.ps1`, `Set-SiteMembersToViewers.ps1` and `Set-MailboxAddresses.ps1` support it. All three still write their log file under `-WhatIf`, because the log is the point of a rehearsal.
...
**Re-running is safe.** The write scripts read current state first and skip anything already correct. Nothing removes a user, a member, or an owner. The single exception is `Set-MailboxAddresses.ps1 -RemoveOldPrimary`, which removes an email address on purpose — that is the step that releases a domain, and it is off unless you ask for it.
